...
class TwitterAppMenu(template.Node):
    ...
    def _procrasdonate_pages(app_page, page_name, context):
        ret = []
        
        if app_page == 'LearnMore':
            ret.append( (page_name, reverse('learn_more'), True) )
        else:
        ...
        if app_page == 'MyImpact':
            ...
            ret.append( ('My Impact', reverse('my_impact'), False) )
            
        if app_page == 'OurCommunity':
            ret.append( (page_name, reverse('our_community'), True) )
        else:
            ret.append( ('Our Community', reverse('our_community'), False) )
    ...
    PAGES = {'imager': _imager_pages,
             #'tcal': _tcal_pages,
             'procrasdonate': _procrasdonate_pages,
             'overview': _overview,
             }
    META_ITEMS = {'imager': ['Help', 'Feedback'],
                  #'tcal': ['About', 'Help', 'Feedback'],
                  'procrasdonate': ['Help', 'Feedback'],
    ...
    def render(self, context):
        ...
        if self.app_name:
            context['apps_menu'] = [('', 'twitter apps:',reverse('overview'),'overview'==self.app_name),
                                    ...
                                    ('<img alt="" src="%stwitter/procrasdonate/img/ProcrasDonateLogo.png" height="30" style="margin-bottom: -1em;">' % settings.MEDIA_URL, 'ProcrasDonate (beta)', reverse('procrasdonate'),' procrasdonate'==self.app_name),
